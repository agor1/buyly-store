...
const benefits = [
  {
    ...
    text: "Realizacja zamówjienia w ciagu 24 godzin roboczych.",
  },
  {
    icon: ShieldCheck,
    title: "Bezpieczny zakup",
    text: "Ochrona płatnosci i przejrzyste zasady zwrotu.",
  },
  {
    icon: Package,
...
export default function ProductDetailsView({ slug }: ProductDetailsViewProps) {
  ...
  useEffect(() => {
    ...
    const loadProduct = async () => {
      try {
        ...
        if (isMounted) {
          setProduct(null);
          setError("Nie udalo sie pobrac produktu.");
        }
      } finally {
        if (isMounted) {
          setIsLoading(false);
  ...
  const specs = product
    ? [
        ...
        { label: "Wysylka", value: product.stock > 0 ? "24h" : "Niedostepna" },
      ]
    : [];
